...
본 문서는 `frontend` 레포 내에 구현된 실제 코드의 아키텍처 패턴을 설명합니다.  
...
- **`hooks/use*Handler.ts`**: UI atom setter + mutation을 엮은 이벤트 핸들러 모음
...
│   └── use*Handler.ts  ⑤ 이벤트 핸들러 (UI atom setter + mutation 조합)
└── components/
