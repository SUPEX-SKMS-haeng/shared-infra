...
│   └── workflows/
│       ├── gemini-review.yml    ← Gemini PR 자동 리뷰
...
1. **1단계: Gemini 자동 리뷰** — `gemini-review.yml`이 트리거되어 기능 충족, 에러 처리, 보안, 코드 품질 등을 자동 검사. PR에 리뷰 코멘트가 달립니다.
2. **2단계: PL 최종 리뷰** — Gemini 리뷰를 참고하여 PL이 Approve/Request Changes.
...
### 6.3 3단계: PR 이후 (Gemini 리뷰)

Gemini PR 리뷰가 코드 품질/포맷을 추가 검사합니다 (1~2단계에서 이미 포맷은 맞춰진 상태).
