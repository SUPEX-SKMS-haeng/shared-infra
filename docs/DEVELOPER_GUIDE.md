...
└── frontend              ← 프론트엔드 (React/Vite)   :3000
```
...
│   └── workflows/
│       ├── claude-review.yml    ← Claude PR 자동 리뷰
...
1. **1단계: Claude 자동 리뷰** — `claude-review.yml`이 트리거되어 기능 충족, 에러 처리, 보안, 코드 품질 등을 자동 검사. PR에 리뷰 코멘트가 달립니다.
2. **2단계: PL 최종 리뷰** — Claude 리뷰를 참고하여 PL이 Approve/Request Changes.
...
### 6.3 3단계: PR 이후 (Claude 리뷰)

Claude PR 리뷰가 코드 품질/포맷을 추가 검사합니다 (1~2단계에서 이미 포맷은 맞춰진 상태).
...
gh secret set ANTHROPIC_API_KEY --org agent-template-apps --body "sk-ant-xxx"
```

- `NOTION_API_KEY`: Notion Integration 토큰
- `NOTION_DATABASE_ID`: 데이터베이스 ID
- `ANTHROPIC_API_KEY`: Claude PR 리뷰용
